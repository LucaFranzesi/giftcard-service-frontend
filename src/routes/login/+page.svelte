<script lang="ts">
	...
	const handleSubmit = async (e: SubmitEvent) => {
		...
		try {
			...
			if (!response.ok) {
				throw new Error(data.message || ($_('LOGIN_FAILED_ERROR') ?? 'Login failed'));
			}

			// Redirect to dashboard
			invalidateAll();
		} catch (err: unknown) {
			error = err instanceof Error ? err.message : ($_('UNEXPECTED_ERROR') ?? 'An unexpected error occurred');
		} finally {
			loading = false;
		}
	};
...
<div class="h-screen flex items-center justify-center overflow-hidden">
	<div class="p-6 max-w-sm bg-surface-100-900 rounded-xl shadow-md space-y-4">
		...
		<h1 class="text-2xl font-bold text-center text-primary">{$_('LOGIN_TITLE') ?? 'Login'}</h1>
		<form onsubmit={handleSubmit} class="space-y-4">
			<div>
				<label for="username" class="block text-sm font-medium">{$_('USERNAME_LABEL') ?? 'Username:'}</label>
			...
			<div>
				<label for="password" class="block text-sm font-medium">{$_('PASSWORD_LABEL') ?? 'Password:'}</label>
			...
			<button type="submit" disabled={loading} class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-primary-50 bg-primary-500 hover:bg-primary-dark focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary">
				{loading ? $_('LOGGING_IN_BUTTON') ?? 'Logging in...' : $_('LOGIN_BUTTON') ?? 'Login'}
			</button>
		</form>
	</div>
</div>